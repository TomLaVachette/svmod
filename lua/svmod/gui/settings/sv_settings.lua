...
concommand.Add("svmod", function(ply)
	CAMI.PlayerHasAccess(ply, "SV_EditOptions", function(hasAccess)
		...
		if hasAccess then
			...
			net.WriteBool(SVMOD.CFG["Damage"]["StopEngineAfterCrash"])
			net.WriteFloat(SVMOD.CFG["Damage"]["StopEngineAfterCrashTime"])
			net.WriteFloat(SVMOD.CFG["Damage"]["StopEngineAfterCrashDamageMultiplier"])
